...
* return : none
* notes  : iono-free LC is defined by internal slots 0 and 1
*           according to the centralized frequency plan in rtkcmn.c
*-----------------------------------------------------------------------------*/
                      ...
                      double *dant)
{
    ...
    freq[0]=sigidx2freq(sys,0,fcn);
    freq[1]=sigidx2freq(sys,1,fcn);
    if (freq[0]==0.0||freq[1]==0.0||fabs(freq[0]-freq[1])<=1E-9) return;

    C1= SQR(freq[0])/(SQR(freq[0])-SQR(freq[1]));
    C2=-SQR(freq[1])/(SQR(freq[0])-SQR(freq[1]));

    satantofffreq(time,rs,sat,nav,dantx,danty,dantz);

    for (i=0;i<3;i++) {
        if      (i==0) dant[i]=C1*dantx[0]+C2*dantx[1];
        else if (i==1) dant[i]=C1*danty[0]+C2*danty[1];
        else           dant[i]=C1*dantz[0]+C2*dantz[1];
    }
}
